"""Runs the scripts for updates every 5 minutes"""
...
from maintenance_jenkins import main as appsecjenkins
from maintenance_snow import every_5_minutes
...
class ScrVar():
    ...
    @staticmethod
    def run_updates_every_5_minutes(main_log=LOG,ex_log=LOG_EXCEPTION,ex_file=EX_LOG_FILE):
        """Runs our updates every 5 minutes"""
        ScrVar.running_function = inspect.stack()[0][3]
        ScrVar.timed_script_setup(main_log,ex_log,ex_file)
        jenkins_updates(main_log,ex_log,ex_file)
        ccsx_updates(main_log,ex_log,ex_file)
...
def jenkins_updates(main_log=LOG,ex_log=LOG_EXCEPTION,ex_file=EX_LOG_FILE):
    """Runs updates for Jenkins"""
    try:
        appsecjenkins()
    except Exception as e_details:
        func = "main() [running appsecjenkins()] from maintenance_jenkins.py"
        e_code = "UEM-JU-001"
        TheLogs.function_exception(func,e_code,e_details,ex_log,main_log,ex_file,ScrVar.src,
                                    inspect.stack()[0][3])
        ScrVar.fe_cnt += 1

def ccsx_updates(main_log=LOG,ex_log=LOG_EXCEPTION,ex_file=EX_LOG_FILE):
    """Runs updates for CCSX tickets"""
    try:
        every_5_minutes()
    except Exception as e_details:
        func = "every_5_minutes() from maintenance_snow.py"
        e_code = "UEM-CU-001"
        TheLogs.function_exception(func,e_code,e_details,ex_log,main_log,ex_file,ScrVar.src,
                                    inspect.stack()[0][3])
        ScrVar.fe_cnt += 1
...
def send_slack_alerts_for_failed_runs(main_log=LOG,ex_log=LOG_EXCEPTION,ex_file=EX_LOG_FILE):
    ...
    try:
        ...
        func = "alert_on_failed_runs('Every5Minutes') from alerts_automation_failures.py"
        e_code = "UEM-SSAFFR-001"
        TheLogs.function_exception(func,e_code,e_details,ex_log,main_log,ex_file,ScrVar.src,
                                    inspect.stack()[0][3])
        ScrVar.fe_cnt += 1
